...
const in_addr_t addresses[NUMBER_OF_ADDRESSES] =
{
...
size_t makeNetflowPacket(char *buffer, int numberOfFlows, time_t systemStartTime)
{
  ...
  for (int flow = 0;flow < numberOfFlows; flow++)
  {
    ...
    record.srcAddr = htonl(pickRandomAddress());
    record.dstAddr = htonl(pickRandomAddress());
    record.nextHop = 0;
    record.input = 0;
    record.output = 0;
    ...
    record.first = (currentTime - systemStartTime - (MIN_DELAY + rand()) % MAX_DELAY)*1000;
    record.last = record.first + (rand() % MAX_DELAY)*1000;
    record.first = htonl(record.first);
    record.last = htonl(record.last);

    record.srcPort = htons(rand() % 65535);
    record.dstPort = htons(rand() % 65535);
    record.pad = 0;
    record.prot = rand() % 2 ? TCP_PROTO : UDP_PROTO;
    record.tcpFlags = record.prot == TCP_PROTO ? randomTCPFlags() : 0;
    record.tos = 0;
    record.srcAs = 0;
    record.dstAs = 0;
    record.srcMask = 0;
